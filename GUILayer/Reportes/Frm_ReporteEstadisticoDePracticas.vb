...
Public Class Frm_ReporteEstadisticoDePracticas
    ...
    Private Sub btn_generar_Click(sender As Object, e As EventArgs) Handles btn_generar.Click
        ...
        Dim consulta = "select T1.nombre as n_practica, COUNT(T2.id_practica) as ctd, (SELECT COUNT (T2.id_practica) * 100/ (SELECT COUNT(T3.id_practica) from DetallesAtencion T3)) as porcentaje FROM DetallesAtencion T2, Practicas T1 where T2.id_practica = T1.id AND fecha_alta BETWEEN CONVERT(datetime, '" + dtp_desde.Value.ToString("MM/dd/yyyy") + "', 103) AND CONVERT(datetime, '" + dtp_hasta.Value.ToString("MM/dd/yyyy") + "',103) GROUP BY T1.nombre"
